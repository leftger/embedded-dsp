...
| **Math, CORDIC & Numerics** | `BFloat16` (50% SRAM buffer reduction), `FloatFloat` (~48-bit double-single extended precision on `f32` FPU), EFT (`two_sum`, `two_prod`, `two_div`), Horner polynomial evaluation & roots, strided dot products (interleaved DMA), CORDIC engine, Complex math, Fast math, Quaternions (`nalgebra` interop), 9 Windows, G.711 $\mu$/A-law companding. |
