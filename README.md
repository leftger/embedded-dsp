...
| Block/lane block processing | ✅ `DspNode`, `Split`/`SplitProcess`, `Lanes`, `Pair`, `Parallel`, `ByLane`, typed `View`/`ViewMut` (`FrameMajor`/`LaneMajor`, `as_layout`) | ✅ same ideas in `dsp-process`, plus chunk bridges and a scratch-buffer `Major` |
...
**Where `idsp` still leads.** Its `dsp-process` crate still has what this one mirrors only in part:
the chunk bridges (`ChunkInOut`, `PerFrame`), a `Buffer` with gap-tolerant streaming, and `Major`'s
stage-major traversal with explicit scratch. It also publishes Python bindings for offline analysis
and filter design. Everything else in the table is either at parity or an `embedded-dsp` advantage —
including the typed view framework, which this crate now carries natively rather than depending on
`dsp-process` for.
