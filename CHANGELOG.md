...
- **Composable Kalman models** (`kalman-models` feature): `embedded_dsp::kalman_models` exposes `idsp`'s type-level Kalman composition — `Estimate`, `Transition`, `Observation`, `Direct`, `RandomWalk`, `ConstantVelocity`, and the `Kalman<P, U>` predictor/updater pairing, with `DenseKalman` as the dense shorthand. Where the existing `kalman` module offers fixed filters you pick from, this lets the prediction and measurement phases be interchangeable types over shared state, including `Kalman::optional()` for measurements that do not arrive every step. The `dsp_process` crate is re-exported alongside it so the filters can be driven through `Split`/`Process` without a second dependency. This is the only feature that pulls external crates (`idsp`, `dsp-process`, `dsp-fixedpoint`) and is deliberately **not** part of `full`, exactly as `nalgebra` is not, so default builds stay dependency-free; it is checked to build for `thumbv7em-none-eabihf` and is covered by `cargo deny` (all four crates are `MIT OR Apache-2.0`). The models are re-exported rather than reimplemented because this component is numerically delicate and upstream maintains it — a hand port would add risk without adding capability.
