...
- **`examples/sr_spike.rs`**: measures `SquareRootKalmanFilter` against `KalmanFilter` and an `f64` reference across eight regimes — up to 200 000 steps, measurement noise from `1e-2` down to `0`, `P₀` anisotropy of `1e12` — reporting the smaller eigenvalue of `P`, its asymmetry, the relative Frobenius distance from the `f64` covariance and the state error against a known trajectory. It is what found the clamp below, and it is the evidence behind the doc rewrite. Run with `cargo run -p embedded-dsp --example sr_spike --release`.

### Fixed

- **`SquareRootKalmanFilter` clamped its covariance to `1e-12`.** `cholesky_inplace_lower` floored every pivot with `.max(1e-12)`, so no factor entry could fall below `1e-6` and no reported covariance entry below `1e-12`, however small the true uncertainty was. A filter tracking `1e-14` therefore reported about `1e-12`: in `examples/sr_spike.rs` the long-run case came out **12.8× too large in its smallest eigenvalue and 116% wrong** in relative Frobenius error against an `f64` reference, and with `R = 0` it reported `1e-12` where the truth is `0`. A non-positive-definite input was indistinguishable from a healthy one for the same reason, because the clamp hid the negative pivot. A non-positive pivot is now **zeroed** — the honest representation of a semi-definite direction, which keeps `P = L Lᵀ` exact and leaves the rank deficiency visible — and the division below it is guarded against a zero diagonal. With the clamp gone the same case lands at f32 roundoff (`5.9e-7`), matches the `f64` reference's eigenvalue in every regime, and retains a tiny covariance direction that the plain `(I − KH)P` form rounds to exactly zero.
- **`SquareRootKalmanFilter`'s documentation claimed a guarantee the code does not implement.** It described "orthogonal Givens transformations" giving a "never diverges due to roundoff" guarantee, but the prediction forms `F S (F S)ᵀ + S_Q S_Qᵀ` as a full matrix and re-factorizes it, and the correction rebuilds `P` from `S`, applies the Joseph form and re-factorizes again — the arithmetic runs on `P`, not on the factor. The doc now says what the filter actually is (factor-*storing*, not a true square-root filter), states which two properties it genuinely has, and points at the example for the measurements.
