...
- **Hyperbolic CORDIC**: `cordic_sqrt_atanh2_q31` and `cordic_atanh_q31` add the hyperbolic vectoring mode, the second of CORDIC's three coordinate systems. `sqrt_atanh2` returns `(sqrt(x² − y²), atanh(y/x))` — the hyperbolic norm and angle, mirroring what `cordic_cartesian_to_polar_q15` returns in the circular case — using shifts and adds only, so `atanh` and `sqrt` need neither a divider nor a multiplier; `atanh` is the same angle as a standalone helper. The 30-entry `atanh(2^-j)` table is constant-folded, and the rotation's `0.8282` gain is pre-compensated in `i64` so the returned norm is the true one. Measured error is `~6e-9` (`≈12` Q31 LSBs) against `f64` `atanh`/`sqrt` across the representable domain. The hyperbolic *rotation* mode (`cosh`/`sinh`) is deliberately absent: `cosh(z) ≥ 1` for every `z`, so a Q1.31 result overflows for any meaningful angle. `idsp`'s linear `mul`/`div` modes were also left out — they do not compute their documented results; see the README note.
