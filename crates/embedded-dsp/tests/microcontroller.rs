//! Consolidated tests: microcontroller_features_tests.

use embedded_dsp::*;

// ─── from microcontroller_features_tests.rs ────────────────────────────────────────
#[test]
...
fn test_hilbert_transform_and_analytic_signal() {
    ...
    // 3. HilbertTransform construction & errors
    let mut state_35 = [0.0f32; 35];
    let mut short_state = [0.0f32; 30];
    assert!(HilbertTransform::<f32>::new(&HILBERT_COEFFS_35, &mut short_state).is_err());
    assert!(HilbertTransform::<f32>::new(&bad_coeffs_even, &mut state_35[..4]).is_err());

    let mut hilbert = HilbertTransform::<f32>::new(&HILBERT_COEFFS_35, &mut state_35).unwrap();
    ...
    assert!(HilbertTransform::<q15>::new(&HILBERT_COEFFS_35_Q15, &mut short_state_q15).is_err());

    let mut hilbert_q15 =
        HilbertTransform::<q15>::new(&HILBERT_COEFFS_35_Q15, &mut state_q15).unwrap();
...
fn hilbert_transform_q15_accumulates_in_i64_and_does_not_overflow() {
    ...
    // accumulator (the hand-written `HilbertTransform` this type replaced used one — see
    ...
    let mut hilbert = HilbertTransform::<q15>::new(&coeffs, &mut state).unwrap();
