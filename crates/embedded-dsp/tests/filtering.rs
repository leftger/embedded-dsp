...
use embedded_dsp::filtering::{
    BiquadCascadeDf2tInstance, BiquadCascadeInstance, CircularBuffer, DcBlockerQ15, FirInstance,
    LmsInstance, NlmsInstance, RecursiveMovingAverage, SinglePoleFilter, biquad_cascade_df1,
    biquad_cascade_df2t, conv_f32, conv_q7, conv_q15, conv_q31, correlate_f32, correlate_q15,
    correlate_q31, fir, lms, lms_leaky, nlms,
};
...
fn fir_f32_q31_q15_run() {
    ...
    let mut qfir = FirInstance::<q31>::init(3, &qcoeffs, &mut qstate);
    fir(&mut qfir, &qsrc, &mut qdst);
    ...
    let mut q15fir = FirInstance::<q15>::init(3, &q15coeffs, &mut q15state);
    fir(&mut q15fir, &q15src, &mut q15dst);
}
...
fn biquad_cascade_variants_run() {
    ...
    biquad_cascade_df1(&mut inst, &src, &mut dst);

    let mut state2 = [0.0f32; 2];
    let mut dst2 = [0.0f32; 4];
    let mut inst2 = BiquadCascadeDf2tInstance::<f32>::init(1, &coeffs, &mut state2);
    biquad_cascade_df2t(&mut inst2, &src, &mut dst2);
    ...
    let mut qdst = [q31::ZERO; 4];
    let mut qinst = BiquadCascadeInstance::<q31>::with_post_shift(1, &qcoeffs, &mut qstate, 0);
    biquad_cascade_df1(&mut qinst, &qsrc, &mut qdst);

    let mut qstate2 = [q31::ZERO; 2];
    let mut qdst2 = [q31::ZERO; 4];
    let mut qinst2 =
        BiquadCascadeDf2tInstance::<q31>::with_post_shift(1, &qcoeffs, &mut qstate2, 0);
    biquad_cascade_df2t(&mut qinst2, &qsrc, &mut qdst2);
    ...
    let mut q15dst = [q15::ZERO; 4];
    let mut q15inst =
        BiquadCascadeInstance::<q15>::with_post_shift(1, &q15coeffs, &mut q15state, 0);
    biquad_cascade_df1(&mut q15inst, &q15src, &mut q15dst);

    let mut q15state2 = [q15::ZERO; 2];
    let mut q15dst2 = [q15::ZERO; 4];
    let mut q15inst2 =
        BiquadCascadeDf2tInstance::<q15>::with_post_shift(1, &q15coeffs, &mut q15state2, 0);
    biquad_cascade_df2t(&mut q15inst2, &q15src, &mut q15dst2);
}
...
fn generic_biquad_cascades_match_reference_bit_for_bit() {
    ...
    for &post_shift in &[0u8, 1u8] {
        ...
        let mut inst =
            BiquadCascadeInstance::<q15>::with_post_shift(1, &coeffs, &mut st, post_shift);
        let mut got_df1 = [q15::ZERO; 8];
        biquad_cascade_df1(&mut inst, &src, &mut got_df1);
        ...
        let mut inst2 =
            BiquadCascadeDf2tInstance::<q15>::with_post_shift(1, &coeffs, &mut st2, post_shift);
        let mut got_df2t = [q15::ZERO; 8];
        biquad_cascade_df2t(&mut inst2, &src, &mut got_df2t);
        assert_eq!(
            got_df2t, want_df2t,
...
fn generic_fir_matches_per_term_reference_bit_for_bit() {
    ...
    let mut inst = FirInstance::<q15>::init(3, &coeffs, &mut st);
    fir(&mut inst, &src, &mut got);
    ...
    let mut finst = FirInstance::<f32>::init(3, &fcoeffs, &mut fst);
    fir(&mut finst, &fsrc, &mut fgot);
...
fn lms_and_nlms_adapt() {
    ...
    let mut lms_inst = LmsInstance::<f32>::init(2, &mut coeffs, &mut state, 0.01);
    lms(&mut lms_inst, &src, &reference, &mut out, &mut err);

    let mut coeffs2 = [0.0f32; 2];
    let mut state2 = [0.0f32; 2];
    let mut lms2 = LmsInstance::<f32>::init(2, &mut coeffs2, &mut state2, 0.01);
    lms_leaky(&mut lms2, &src, &reference, &mut out, &mut err, 0.001);

    let mut coeffs3 = [0.0f32; 2];
    let mut state3 = [0.0f32; 2];
    let mut nlms_inst = NlmsInstance::<f32>::init(2, &mut coeffs3, &mut state3, 0.01, 1e-5);
    nlms(&mut nlms_inst, &src, &reference, &mut out, &mut err);
}
...
fn convolution_correlation_and_misc_filters() {
    ...
    avg.process(1.0);
    avg.reset();
    let mut avgq = RecursiveMovingAverage::<q15, 4>::default();
