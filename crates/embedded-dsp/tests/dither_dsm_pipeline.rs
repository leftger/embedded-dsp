//! Consolidated tests: dither_dsm_pipeline_coverage.
...
// ─── from dither_dsm_pipeline_coverage.rs ────────────────────────────────────────
// ─────────────────────────────────────────────────────────────────────────────
// dither
// ─────────────────────────────────────────────────────────────────────────────
...
fn built_in_nodes_delegate_to_their_inherent_process() {
    use embedded_dsp::controller::PidInstance;
    use embedded_dsp::filtering::{DcBlockerQ15, SinglePoleFilter};
    use embedded_dsp::types::{q15, q31};

    let mut via_node = PidInstance::<f32>::new(1.0, 0.1, 0.01);
    let mut direct = PidInstance::<f32>::new(1.0, 0.1, 0.01);
    assert_eq!(
    ...
    let mut via_node =
        PidInstance::<q15>::new(q15::from_bits(100), q15::from_bits(10), q15::from_bits(1));
    let mut direct =
        PidInstance::<q15>::new(q15::from_bits(100), q15::from_bits(10), q15::from_bits(1));
    assert_eq!(
    ...
    let mut via_node =
        PidInstance::<q31>::new(q31::from_bits(100), q31::from_bits(10), q31::from_bits(1));
    let mut direct =
        PidInstance::<q31>::new(q31::from_bits(100), q31::from_bits(10), q31::from_bits(1));
    assert_eq!(
